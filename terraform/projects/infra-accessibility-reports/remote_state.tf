...
variable "remote_state_infra_mirror_key_stack" {
  type        = "string"
  description = "Override stackname path to infra_mirror remote state "
  default     = ""
}
...
data "terraform_remote_state" "infra_mirror" {
  backend = "s3"

  config {
    bucket = "${var.remote_state_bucket}"
    key    = "${coalesce(var.remote_state_infra_mirror_key_stack, var.stackname)}/infra-mirror.tfstate"
    region = "${var.aws_region}"
  }
}
